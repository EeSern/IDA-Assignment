...
with tab1:
    ...
    with col1:
        ...
        if selected_rating and charts_data is not None:
            rating_satisfaction = charts_data.groupby(selected_rating)['Satisfaction_Numeric'].value_counts(normalize=True).mul(100).rename('percent').reset_index()
            rating_satisfaction = rating_satisfaction[rating_satisfaction['Satisfaction_Numeric'] == 1]
    ...
    with col2:
        ...
        if charts_data is not None:
            ...
            segment_satisfaction = charts_data.groupby(segment_col)['Satisfaction_Numeric'].mean().mul(100).reset_index(name='Satisfied_pct')
    ...
    st.info("Based on these insights, the airline should prioritize **improving the digital experience (Online Boarding, Wi-Fi)** and develop **targeted strategies for the less-satisfied leisure and economy class travelers**.")
...
with tab2:
    ...
    if st.session_state.show_prediction:
        ...
        if prediction[0] == 1:
            ...
            st.metric("Confidence Score (Satisfied)", f"{prediction_proba[0][1]:.2%}")
        else: # If prediction is Dissatisfied (0)
            st.error("This passenger is likely to be **NEUTRAL OR DISSATISFIED** ❌")
            st.metric("Confidence Score (Dissatisfied)", f"{prediction_proba[0][0]:.2%}")
...
with tab3:
    ...
    if uploaded_file is not None:
        try:
            ...
            results_df['Predicted_Satisfaction'] = ["Satisfied" if p == 1 else "Neutral or Dissatisfied" for p in predictions]
            results_df['Probability_Satisfied'] = [f"{p[1]:.2%}" for p in predictions_proba]
